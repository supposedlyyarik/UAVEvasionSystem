...
class UAVGuidance:
    ...
    def _analyze_pattern(self, track_id):
        ...
        if abs(avg_angle_change) > 2:  # Постійна зміна кута > 2°
            return 'turning'
    ...
    def _predict_with_uncertainty(self, track_id, current_pos, current_vel, pattern):
        ...
        if pattern == 'straight':
            ...
            # Зигзаг - важко передбачити, велика невизначеність
            ...
            # Невідомий патерн - максимальна обережність
            predicted_x = tx + vx * time_to_threat
            predicted_y = ty + vy * time_to_threat
